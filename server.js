...
app.post('/api/loyalty/redeem', (req, res) => {
  try {
    ...
    if (!rewardId) {
      return res.status(400).json({ sukses: false, error: "Field 'rewardId' wajib diisi." });
    }
...
app.post('/api/mira/pathway/set-phase', (req, res) => {
  try {
    const { mpiId = 'MPI-0001', pathwayId = 'pasca_pci_jantung', targetDay = 3 } = req.body;
    const data = setPatientPathwayPhase(mpiId, pathwayId, parseInt(targetDay, 10));
    res.json({ sukses: true, pesan: `Pathway & hari pemulihan berhasil diatur ke hari H+${targetDay}`, data });
  } catch (err) {
    res.status(500).json({ sukses: false, error: err.message });
  }
});
...
app.post('/api/mira/checkin', (req, res) => {
  try {
    const { mpiId = 'MPI-0001', pathwayId, phaseId, responseOption, patientNotes = '' } = req.body;
    if (!pathwayId || !phaseId || !responseOption) {
      return res.status(400).json({ sukses: false, error: "Field 'pathwayId', 'phaseId', dan 'responseOption' wajib diisi." });
    }
    const hasil = submitCheckinResponse(mpiId, pathwayId, phaseId, responseOption, patientNotes);
    res.json(hasil);
  } catch (err) {
    res.status(500).json({ sukses: false, error: err.message });
  }
...
app.post('/api/mira/nurse-queue/action', (req, res) => {
  try {
    const { queueId, actionType, notes = '', nurseName = 'Ns. Ratih Wardani, S.Kep' } = req.body;
    if (!queueId || !actionType) {
      return res.status(400).json({ sukses: false, error: "Field 'queueId' dan 'actionType' wajib diisi." });
    }
...
app.get('/api/mira/templates', (req, res) => {
  try {
    res.json({ sukses: true, templates: Object.values(CARE_PATHWAY_TEMPLATES) });
  } catch (err) {
    res.status(500).json({ sukses: false, error: err.message });
  }
});
...
app.get(['/unnamed.png', '/assets/unnamed.png', '/assets/care-dokter-logo.png', '/care-dokter-logo.png'], (req, res) => {
  const possiblePaths = [
